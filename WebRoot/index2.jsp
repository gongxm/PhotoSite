...
//设置
//LUCY, lucy的页面名称
private static final String LUCY = "index2.jsp";
//EDITOR,文本编辑器的页面名称;
...
/*
pj_crFolder(String path,String name),创建文件夹
...
//error
if(success){resultInfo="<div class='success'>1个文件夹已创建:["+crPath+"]</div>";}
else{resultInfo ="<div class='error'>0个文件夹已创建,不能创建:["+crPath+"]."+fileExistsError+"</div>";}
return (sysInfo+resultInfo);
}//end function
/*
pj_crFile(String path,String name),创建文件
...
//error
if(success){resultInfo="<div class='success'>1个文件已创建:["+crPath+"]</div>";}
...
<div id="about">
<a href='#' onClick="about();">关于</a>&nbsp;|&nbsp;<a href='help.html' target="_blank">帮助</a>
</div>
<hr>
<%
String resultInfo="";
...
filesize=ff.length();
out.print("<div class='success'>1个文件已上传:["+savepathname+"],size:["+pj_convertFileSize(filesize)+"("+filesize+"字节)]</div>");
}//else
}//else
}//else
}//else
...
<tr><td colspan="2">驱动器:<%=pj_getDrivers()%></td></tr>
<tr><td colspan="2">当前路径:<%=pj_path2link(path)%><br></td></tr>
<tr><td>
<%
if(objFile.getParent()==null){out.print("<a href='#' disabled>已是最顶层目录</a> |");}
else{out.print("<a href='"+LUCY+"?path="+URLEncoder.encode(objFile.getParent())+"'>上一级目录</a>  |");}
...
<b>移动文件(Move File)</b><br>
移动选中的文件到你填写的目标路径下<br>
...
<b>复制文件(Copy File)</b><br>
复制选中的文件到你填写的目标路径下<br>
...
<b>删除文件(Delete File)</b><br>
这是一项危险的操作,文件一旦被删除就不能恢复了<br>
强烈建议你重新检查一次选中的文件,或者使用重命名代替删除.<br>
如果你确定要这么做,点击下面按钮,选中的文件将被彻底删除.<br>
...
<b>上传文件(Upload)</b><br>
你将上传一个文件到:<font color=red><%=path%></font><br>
...
<b>创建一个新文件夹(Create Folder)</b><br>
你将创建一个新文件夹在:<font color=red><%=path%></font><br>
<form method="POST" action="<%=LUCY%>">
<input type="hidden" name="path" value="<%=path%>">
please enter新文件夹的名称:(不要含有 \ / : * ? " < > |)<br>
...
<b>创建一个新文件(Create File)</b><br>
你将创建一个新文件在:<font color=red><%=path%></font><br>
<form method="POST" action="<%=LUCY%>">
<input type="hidden" name="path" value="<%=path%>">
please enter新文件的名称:
(如"abc.txt",并且不要含有 \ / : * ? " < > |)<br>
...
<b>重命名一个文件夹或文件(Rename)</b><br>
你将重命名一个文件夹或文件在:<font color=red><%=path%></font><br>
<form method="POST" action="<%=LUCY%>">
<input type="hidden" name="path" value="<%=path%>">
